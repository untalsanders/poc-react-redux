...
export const Qoutes = () => {
    const [numberOfQoutes, setNumberOfQoutes] = useState(10)

    return (
        <>
            <div>
                <h3>Select the Quantity o Quotes to Fetch:</h3>
                <select
                    className={styles.select}
                    value={numberOfQoutes}
                    onChange={e => {
                        setNumberOfQoutes(Number(e.target.value))
                    }}
                >
                    {options.map(option => (
